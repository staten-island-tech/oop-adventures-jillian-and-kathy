...
class FirstRoom4:
    ...
    def enter(self, inventory):
        slow_print("You have recieved a key!", self.key)
        time.sleep(1)
        self.key = Key("First Room Key")
        inventory.append(self.key)
        print("Now you can unlock the door to the next room.", 0.05)
...
class FirstRoom4:
    def __init__(self):
        self.key = Key(slow_print("First Room Key", 0.05))
